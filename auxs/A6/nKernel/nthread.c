...
static nThread nth_makeThread(int stackSize) {
  ...
  newTh->joinTh= NULL; // Ninguna tarea ha hecho nWaitTask
  ...
  if (stackSize==0) {
    ...
    newTh->sp= &newTh->stack[stackSize/sizeof(void *)-1];
...
void dumpThreads(void) {
  ...
  while (mapHasNext(iter)) {
    ...
    printf("thread=%p %s core=%d died=%d next=%p join=%p\n",
           (void*)th, statusName, th->allocCoreId, th->freeIt,
...
int nJoin(nThread th, void **pRetPtr) {
  ...
  if (nth_allocCoreId(th)<0) {
    ...
    // It thread th is allocated to some core, it cannot be freed yet,
    // the allocated core will free it when transferred to another thread
    th->freeIt= 1;
  }
...
int main(int argc, char *argv[]) {
  ...
  for (int in=1; in<argc; in++) {
    if (strcmp(argv[in],"-slice")==0 && ++in<argc) {
      ...
      printf("Choose -ncores <n> -slice <time>, with n>=2 for multi-core"
...
void nth_changeContext(nThread thisTh, nThread nextTh) {
  ...
  if (thisTh!=NULL) {
    ...
    if (thisTh->freeIt) {
      // It should have been freed in nJoin, but it could not because
      // it was still allocated to thisTh->allocCoreId
    if (thisTh->stack!=NULL)
       free(thisTh->stack);
    free(thisTh);
    psp= &dummySp;
    }
  ...
  _ChangeToStack(psp, &nextTh->sp); // In assembler
...
void nth_callInNewContext(nThread thisTh, nThread newTh,
  ...
  _CallInNewStack(&thisTh->sp, newTh->sp, proc, ptr); // In assembler
